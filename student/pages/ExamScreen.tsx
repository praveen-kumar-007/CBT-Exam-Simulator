                ...
                <aside className="flex min-h-0 flex-col gap-4 overflow-y-auto">
                    <section className="rounded-2xl border border-slate-300 bg-white p-4 shadow-md">
                        <div className="border-b border-slate-200 pb-2">
                            ...
                            <p className="mt-1 text-xs text-slate-500">Compact section navigator</p>
                        </div>
                            ...
                            {examData.sections.map((section, idx) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full rounded-md border px-2.5 py-2 text-left transition ${isActive ? 'border-sky-600 bg-sky-50 shadow-sm' : 'border-slate-300 bg-white hover:border-slate-400 hover:bg-slate-50'}`}
                                    >
                                        <div className="flex items-center justify-between gap-2">
                                            <span className="max-w-[68%] truncate text-xs font-semibold text-slate-900">
                                                S{idx + 1}. {section.name}
                                            </span>
                                            <span className="rounded border border-emerald-300 bg-emerald-100 px-2 py-0.5 text-[11px] font-semibold text-emerald-800">
                                                {counts.answered}/{section.questions.length}
                                            </span>
                                        </div>
                                    </button>
                                );
                            ...
                            {currentSection.questions.map((q, index) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`h-10 rounded-md text-sm font-semibold transition ${style} ${currentQuestionIndex === index ? 'ring-2 ring-blue-500 ring-offset-2' : ''}`}
                                    >
                                        {index + 1}
                                    </button>
                                );
